...
do n = 11, num_points, 10
  ...
  ! Calcola l'errore
  error = abs(q_simp - exact_value)
  
  Terror = abs(q_trap - exact_value)
  
  ! Scrivi log10(N), log10(error) nel file dati
  write(2, *) log10(real(n)), log10(error), , log10(Terror) 
end do
 close(2)

!come è possibile apprezzare dai dati presenti in data.txt l'errore sembra diminuire esponenzialmente all'aumentare del numero di punti inclusi nel calcolo dell'integrale
